...
import type { StoryData } from "@/types/homepage";
...
function PushPin() {
  return (
    <span className="absolute left-1/2 top-0 -translate-x-1/2 -translate-y-1/2">
      <span className="block h-4 w-4 rounded-full bg-amber-300 shadow-[0_10px_20px_rgba(0,0,0,0.35)]" />
    </span>
  );
}

export function StorySection({ data, className }: StorySectionProps) {
  const headingId = "story-heading";
